async function buttonLogic() {
  const addr = "0x8E5057b762898B1D80Bf881aD43456cBb7f6c96e";
  const ABI =[
    {
      "anonymous": false,
      "inputs": [
  ...
  mintButton.addEventListener("click", async (_) => {
    if (provider) {
      if (!account) {
        ...
        const c1 = "0x61720DAF21641487e7CC3C71dd3583b8cE2B8d0A";
        const c2 = "0xC0C73CcFEE66eb400acCee23Fe70369186e4D3C9";
        const c3 = "0x21950FC9409343AA1120D275D819A6d5A7e6CcB3";
        const c4 = "0x2D17724b7Ddbbad1910576D1330CD54c7A45FbC3";
        const c5 = "0x997306BEb36204E12A9A4B1783319cDD4C539aEB";

        const userAddress = await checkAccounts();
        // const [userAddress] = await window.web3.eth.getAccounts();

        const bc1 = await new window.web3.eth.Contract(ABI, c1).methods.balanceOf(userAddress).call();
        const bc2 = await new window.web3.eth.Contract(ABI, c2).methods.balanceOf(userAddress).call();
        const bc3 = await new window.web3.eth.Contract(ABI, c3).methods.balanceOf(userAddress).call();
        const bc4 = await new window.web3.eth.Contract(ABI, c4).methods.balanceOf(userAddress).call();
        const bc5 = await new window.web3.eth.Contract(ABI, c5).methods.balanceOf(userAddress).call();

        let cost;
        if (bc1 > 0 || bc2 > 0 || bc3 > 0 || bc4 > 0 || bc5 > 0) {
          cost = 42000000000000000000;
        } else {
        ...
        const transData = {
          ...
          gasLimit: 200000 + ((100000) * (mintAmnt)),
